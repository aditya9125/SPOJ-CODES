...
int main()
{
	...
	while(t--)
	{
		...
		for(int i=1;i<=k;i++)		/// to choose exact i kg apples
		{

			dp[i][1]=(price[i]!=-1)? price[i] :LLmax ;


			for(int j=2;j<=n;j++)		///in at most j picks
			{
				...
				for(int x=i;x>(i>>1);x--)
				{
					...
					if(price[x]!=-1 and dp[i-x][j-1]!= LLmax){

						 dp[i][j] = min(dp[i][j], price[x] + dp[i-x][j-1]) ;	///taking x kg in one pick + F(i-x, j-1)
