///Status : AC
///Remark :	don't know where its going wrong
...
public:
	...
	graph(int n){ adj= vector<VII >(n);	visited= VI(n,0);}

	void add_dedge(int x,int y,int cost=1){ 	adj[x].pb({y,cost}); }

	void add_uedge(int x,int y,int cost=1){ 	adj[x].pb({y,cost}); adj[y].pb({x,cost}); }
	...
	while(e--)
	{
		...
		while(!q.empty())
		{
			...
			for(int j=0;j<G.adj[p].size();j++)
			{
				...
				int v= G.adj[p][j].F;
				int edgwt = G.adj[p][j].S;

                if(dist_to_p!=INTmax and  dist_to_p+ edgwt < cost[v])
                {

					cost[v] = dist_to_p + edgwt;
