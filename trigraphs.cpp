...
int main()
{
	...
	while(true)
	{
		...
		dp[0][0]=INTmax,dp[0][1]=m[0][1],dp[0][2]=m[0][1]+m[0][2];

		for(int i=1;i<n;i++)
		{
			dp[i][0]=m[i][0]+ min(dp[i-1][0],dp[i-1][1]);

			dp[i][1]=min4(dp[i-1][1] , dp[i-1][2], dp[i-1][0],dp[i][0]) + m[i][1] ;

			dp[i][2]=min3(dp[i-1][2],dp[i-1][1],dp[i][1])+ m[i][2];

		}

		cout<<test<<". "<<dp[n-1][1]<<nl;
	}
