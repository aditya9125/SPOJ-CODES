...
string add(string a,string b)
{
	...
	reverse(whole(a)); reverse(whole(b));
...
string bigMul(int lx,int rx,int ly,int ry)
{
	...
	string AC= bigMul(lx,midx,ly,midy) ;

	append(n,AC);

	string AD = bigMul(lx,midx,midy+1,ry);

	append(n/2,AD);

	string BC= bigMul(midx+1,rx,ly,midy) ;

	append(n/2,BC);

	string BD = bigMul(midx+1,rx,midy+1,ry) ;

	string ans =  add(add(AC,AD),add(BC,BD));
...
int main()
{
	...
	//till here size of both a and b is equal

	if((a.size() & (a.size()-1))!=0){

		int fac=log2(a.size()) +1;

		int reqsize= pow(2,fac);

		int extra= reqsize - a.size();

		string tempa,tempb;

		for(int i=0;i<extra;i++) tempa.pb('0'),tempb.pb('0');
	...
	string ans = bigMul(0,a.size()-1,0,b.size()-1);
	...
	for(int j=i;j<ans.size();j++) newans.pb(ans[j]);

	if(newans.size()==0) newans= "0";
