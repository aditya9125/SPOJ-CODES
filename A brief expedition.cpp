...
void dijkstra(int si,int sj,int r,int c)
{

	visited= VVI(r,VI(c,0));

	dist= VVI(r,VI(c,INTmax));
...
int main()
{
	...
	//ofstream cout("/home/aditya/Documents/op");
		...
		int t;	cin>>t;while(t--){
				...
				{
					ans = ans + dist[item.F][item.S]*2 + 60;
				}
				...
				for(auto item:stores){
					...
					maxdist = max(maxdist, (ll)dist[item.F][item.S]);
				}
