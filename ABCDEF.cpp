...
int main()
{
	...
	for(int i=0;i<vec.size();i++)
	{
		...
		for(int j=0;j<vec.size();j++)
		{
			...
			{
				if(vec[k])
					RHS.pb( vec[k]* (vec[i]+ vec[j]) );
			}
		}
	}


	sort(whole(LHS)),sort(whole(RHS));
	...
	for(auto lit=LHS.begin();lit!=LHS.end();)
	{
			...
			auto nit=upper_bound(whole(LHS),*lit);


			auto lo= lower_bound(whole(RHS),*lit);
			auto hi= upper_bound(whole(RHS),*lit);

			ans = ans + (hi-lo)*(nit-lit);
			lit= nit;
