...
int main()
{
	...
	VVI table(101,VI(5000)),psum(101,VI(5000));


	for(int i=1;i<=100;i++)
		table[i][0]=1,psum[i][0]=1;

	table[2][1]=1;

	table[3][1]=2,table[3][2]=2,table[3][3]=1;

	for(int i=1;i<=12;i++)
		for(int k=1; k<= (i*(i-1))/2;k++)
			psum[i][k] = table[i][k] + psum[i][k-1];
	...
	for(int i=4;i<=12;i++)
	{
		for(int k=1;k<=(i*(i-1))/2;k++)
		{
			//int prev_max=(i-1)*(i-2)/2;
			//if(k>prev_max)	s = k-prev_max,e=prev_max;

			int e= min(k,(i-1)*(i-2)/2);

			if(k>i-1)	s=  k-i+1;
			else s=0;

//			int sum=0;			for(int j=s;j<=k;j++)			 sum+= table[i-1][j];			table[i][k]=sum;
			table[i][k]= psum[i-1][e]- ( (s>0)?psum[i-1][s-1]:0);

			psum[i][k]= psum[i][k-1] + table[i][k] ;
			//cout<<i<<" "<<k<<nl;
		}
	}
