...
void bfs(int x,int y)
{
		...
		while(!q.empty())
		{
			...
			ii top=q.front();
			int h=top.F,k=top.S;
			int dist=1+opdist[h][k];

			q.pop();

			if(k-1>=0 and dist<opdist[h][k-1])
			{
				q.push({h,k-1});
				opdist[h][k-1]=dist;
				//visited[h][k-1]=true;
			...
			if(k+1<m and dist<opdist[h][k+1])
			{
				q.push({h,k+1});
				opdist[h][k+1]=dist;
				//visited[h][k+1]=true;
			}
			if(h-1>=0 and dist<opdist[h-1][k])
			{
				q.push({h-1,k});
				opdist[h-1][k]=dist;
				//	visited[h-1][k]=true;
			...
			if(h+1<n and dist<opdist[h+1][k])
			{
				q.push({h+1,k});
				opdist[h+1][k]=dist;
				//visited[h+1][k]=true;
...
while(t--)
{
		...
		rep(i,0,n)
		{
			...
			rep(j,0,m)
				pred[i][j]? cout<<"0 ":cout<<opdist[i][j]<<" ";
			newl;
		}
