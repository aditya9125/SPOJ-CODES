...
bool dfs(int i,int j,int k)
{
	...
	for(int x=0;x<3;x++){
		for(int y=0;y<3;y++){
			...
			int ii=i+pick[x],jj=j+pick[y];
			if(ii==i and jj==j) continue;


			if(ii>=0 and ii<r and jj>=0 and jj<c and !visited[ii][jj])
			{
				//cout<<ii<<","<<jj<<" : "<<i<<" "<<j<<" k= "<<k<<nl;
					if(grid[ii][jj]==targ[k]) dfs(ii,jj,k+1);
					visited[ii][jj]=0;
			}
		}
	}
