...
class Graph
{
  public:
	...
	Graph(int n){		this->n = n, visited= VB(n+1,false), adj= vector<VII> (n+1); maxpath =-1; maxnode =0;}

		void uedge(int x, int y,int wt=1) {adj[x].push_back({y,wt}),adj[y].push_back({x,wt});  }
		void dedge(int x,int y,int wt=1) { adj[x].pb({y,wt});}

		void dfs(int ,ll);
		//void wedge(int v,int w,int weight)
};


void Graph::dfs(int v,ll dist)
{
	visited[v]=1;

	if(dist> maxpath)
		maxpath =dist, maxnode = v;

	for(int j=0;j<adj[v].size();j++)
	{

		int cv = adj[v][j].F; ll cost = adj[v][j].S;
...
//            cout<<"maxpath = "<<g.maxpath<<"maxnode = "<<g.maxnode<<nl;

			Graph newg(n);

            newg.adj = g.adj;

			newg.dfs(g.maxnode,0);

			cout<<newg.maxpath<<nl;
