...
void dfs(int i,int j,int dist)
{
	...
	for(int k=0;k<4;k++)
	{
		...
		if(isvalid(ii,jj) and !tab[ii][jj] and !visited[ii][jj]){
...
int main()
{
	...
	while(t--)
	{
		...
		//cout<<fnode.F<<' '<<fnode.S<<nl;
		//cout<<"max dist after first dfs "<<maxdist<<nl;

		maxdist=0;


		FILL_MAT(visited,r,c,0);

		//cout<<fnode.F<<" "<<fnode.S<<nl;
		dfs(fnode.F,fnode.S,0);

		cout<<"Maximum rope length is "<<maxdist<<"."<<nl;
