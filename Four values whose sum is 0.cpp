...
//      for(auto it= AB.begin();it!=AB.end();it++) cout<<it->F<<" "<<it->S<<nl;


//		for(auto it= CD.begin();it!=CD.end();it++) cout<<it->F<<" "<<it->S<<nl;
		...
		for(int i=0;i<a.size();i++)
		{
			...
			if(binary_search(whole(b),-a[i]))
			{
				auto bounds= equal_range(whole(b),-a[i]);

				ans = ans + (bounds.S-b.begin()) - (bounds.F-b.begin());
			}
		}
