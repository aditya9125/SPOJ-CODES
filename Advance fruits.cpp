...
void lcs( string &X, string &Y,string & result)
{
   ...
   while (i > 0 && j > 0)
   {
      if (X[i-1] == Y[j-1])
      {
          result.pb(X[i-1]); // Put current character in result
...
void print(string const & A, string const& B ,string const &LCS)
{
	...
	while(i<A.size())
	{
		...
		if(A[i]==B[j] and A[i]==LCS[k])
		{
			cout<<A[i];
			i++,j++,k++;
		}
		else if(A[i]!=LCS[k] and B[j]!=LCS[k])
		{
			cout<<A[i];
			++i;
		}
		...
		else
		{
            ...
            if(A[i]==LCS[k])
            {
				...
				while(j<B.size() and B[j]!=LCS[k])
					cout<<B[j++];

				if(j<B.size()) cout<<LCS[k];
			}
			else if(B[j]==LCS[k])
			{

				while(i<A.size() and A[i]!=LCS[k])
					cout<<A[i++];

				if(i<A.size()) cout<<LCS[k];
			}
