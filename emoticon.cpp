...
int main()
{
	...
	//ofstream cout("/home/aditya/Documents/op");
		...
		int t;	cin>>t;while(t--){
			...
			string S;
			cin>>S;
			//for(int i=0;i<S.size();i++) if(islower(S[i])) S[i]=(char)toupper(S[i]);
            ...
            for(int i=n-2;i>=0;i--)	karr[i]= karr[i+1]+(S[i]=='K');
            ...
            for(int i=n-2;i>=0;i--)
            {
				...
				if(S[i]=='K') dp[i]= earr[i+1];
				else if(S[i]=='E') earr[i]= earr[i+1] + karr[i+1];
...
#ifdef debug1
			for(int i=0;i<n;i++) cout<<karr[i]<<" "; newl;
			for(int i=0;i<n;i++) cout<<earr[i]<<" "; newl;
			for(int i=0;i<n;i++){
											if(S[i]=='K')
												cout<<dp[i]<<" ";
											else cout<<"* ";
