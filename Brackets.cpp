...
void build(int l,int r,int v)
{
	...
	int pairs= min(tree[2*v].uopen,tree[2*v+1].uclosed);

	tree[v].uopen= tree[2*v].uopen+ tree[2*v+1].uopen- pairs;
	tree[v].uclosed=tree[2*v].uclosed+ tree[2*v+1].uclosed- pairs;
}

void update(int l,int r,int indx,int v)
{

	if(l==indx and r==indx)
	{
		flip(indx);
		swap(tree[v].uclosed,tree[v].uopen);
		return;
	}
	...
	update(l,mid,indx,2*v);
	update(mid+1,r,indx,2*v+1);
...
//int t;	cin>>t;
	...
	for(int test=1;test<=10;test++)
	{
		...
		while(q--)
		{
			...
			//cout<<tree[1].uopen<<" "<<tree[1].uclosed;

				if(tree[1].uopen==0 and tree[1].uclosed==0) cout<<"YES";
				else cout<<"NO";
				newl;

			}else flip(x-1),update(0,n-1,x-1,1);
