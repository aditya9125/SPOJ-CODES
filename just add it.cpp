...
class CompareDist
{
public:				///pairType doesn't name a T ,its just to show T of pair object passed in operatorfunction
...
template<typename T>
void get_arr(T a[],T n)
{
for(T i=0;i<n;i++) cin>>a[i];
}

template<typename T>
void show_arr(T a[],T n=1) {for(T i=0;i<n;i++) cout<<a[i]<<" ";}

template<typename T>
inline T max3(T a,T b,T c){	 return max(max(a,b),c);}

template<typename T>
inline T min3(T a,T b,T c){	 return min(a,min(b,c));}

template<typename T>
inline T min4(T a,T b,T c,T d){	 return min(a,min3(b,c,d));}

template<typename T>
inline T max4(T a,T b,T c,T d){	 return max(a,max3(b,c,d));}

template<typename T>
T phi(T n){ T result = n;  for (T p=2; p*p<=n; ++p) { if (n % p == 0){ while (n % p == 0)   n /= p; result -= result / p; } } if (n > 1) result -= result / n; return result; }

template<typename T> T gcd(T A,T B){	if(B==0)	return A;	else return gcd(B, A % B);	}

template<class T>
T ncr(T n,T r){  	assert(n>=r); T ans=1,i; 	if(n==r) return 1; if(r==1) return n; if(r>(n-r)) r=n-r; reptill(i,0,r-1) ans=ans*(n-i)/(i+1);	return ans; }

template<typename T>  ull fact(T x){ ull ans=1; for(T i=1;i<=x;i++){ ans*=i;        /*ans%=MOD;   /Optional*/ } return ans; }

template<typename T>
int Digs(T n){ return (n >= 10) ? 1 + Digs(n/10): 1; }

template<typename T>
T tonum(string s){	T ans=0,i;	rep(i,0,s.size())	ans=ans*10+(s[i]-'0'); return ans; }
...
#define MOD 10000007
template<typename T>
T modpowIter(T a, T b, T c= MOD) { T ans=1;  while(b != 0){ if(b%2 == 1) ans=(ans*a)%c;  a=(a*a)%c; b /= 2; } return ans; }
...
int main()
{
	...
	while(true)
	{
		...
		ll ans = ( modpowIter(n,k) % MOD + (2*modpowIter(n-1,k )) %MOD + modpowIter(n,n) %MOD + (2* modpowIter(n-1,n-1))%MOD ) %MOD;
